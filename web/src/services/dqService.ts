...
export const dqService = {
  // 创建规则
  createRule: (data: DQRuleCreate) => api.post<DQRule>('/dq/rules', data),
  ...
  // 获取规则详情
  getRule: (id: string) => api.get<DQRule>(`/dq/rules/${id}`),
  ...
  // 删除规则
  deleteRule: (id: string) => api.delete<void>(`/dq/rules/${id}`),
  ...
  // 获取统计数据
  getStats: () => api.get<DQStats>('/dq/stats'),
};
