...
// CreateTag 创建标签
func (s *SQLiteStore) CreateTag(ctx context.Context, tag *TagCreate) (string, error) {
	query := `
		INSERT INTO tags (id, name, color, description, created_at, updated_at)
		VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))
	`
	id := uuid.New().String()
	_, err := s.db.ExecContext(ctx, query, id, tag.Name, tag.Color, tag.Description)
	if err != nil {
...
// GetTag 根据ID获取标签
func (s *SQLiteStore) GetTag(ctx context.Context, id string) (*TagRow, error) {
	query := `
...
func (s *SQLiteStore) GetTagByName(ctx context.Context, name string) (*TagRow, error) {
	query := `
		SELECT id, name, color, description, created_at, updated_at
...
// ListTags 列出所有标签
func (s *SQLiteStore) ListTags(ctx context.Context) ([]*TagRow, error) {
	query := `
		SELECT id, name, color, description, created_at, updated_at
...
// UpdateTag 更新标签
func (s *SQLiteStore) UpdateTag(ctx context.Context, id string, updates *TagUpdate) error {
	query := `
		UPDATE tags SET
			name = COALESCE(?, name),
		...
		WHERE id = ?
	`
	result, err := s.db.ExecContext(ctx, query, updates.Name, updates.Color, updates.Description, id)
	if err != nil {
...
func (s *SQLiteStore) AddTagToColumn(ctx context.Context, columnID string, tagID string) error {
	query := `
		INSERT OR IGNORE INTO column_tags (id, column_id, tag_id, created_at)
		VALUES (?, ?, ?, datetime('now'))
	`
	id := uuid.New().String()
	_, err := s.db.ExecContext(ctx, query, id, columnID, tagID)
	if err != nil {
...
func (s *SQLiteStore) RemoveTagFromColumn(ctx context.Context, columnID string, tagID string) error {
	query := `DELETE FROM column_tags WHERE column_id = ? AND tag_id = ?`
	result, err := s.db.ExecContext(ctx, query, columnID, tagID)
	if err != nil {
...
func (s *SQLiteStore) GetColumnTags(ctx context.Context, columnID string) ([]*TagRow, error) {
	query := `
...
func (s *SQLiteStore) SearchColumnsByTag(ctx context.Context, tagID string, limit int) ([]*ColumnSearchRow, error) {
	if limit <= 0 {
		limit = 20
	}
	query := `
