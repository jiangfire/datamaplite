...
// CreateTag 创建标签
func (t *PostgresTxStore) CreateTag(ctx context.Context, tag *TagCreate) (string, error) {
	query := `
	...
	err := t.tx.QueryRow(ctx, query, id, tag.Name, tag.Color, tag.Description).Scan(&returnedID)
	if err != nil {
...
// GetTag 根据ID获取标签
func (t *PostgresTxStore) GetTag(ctx context.Context, id string) (*TagRow, error) {
	query := `
		SELECT id, name, color, description, created_at::text, updated_at::text
...
func (t *PostgresTxStore) GetTagByName(ctx context.Context, name string) (*TagRow, error) {
	query := `
		SELECT id, name, color, description, created_at::text, updated_at::text
		FROM tags WHERE name = $1
	`
	row := t.tx.QueryRow(ctx, query, name)
	var tag TagRow
	err := row.Scan(&tag.ID, &tag.Name, &tag.Color, &tag.Description, &tag.CreatedAt, &tag.UpdatedAt)
	if err != nil {
		if err == pgx.ErrNoRows {
...
// ListTags 列出所有标签
func (t *PostgresTxStore) ListTags(ctx context.Context) ([]*TagRow, error) {
	query := `
		SELECT id, name, color, description, created_at::text, updated_at::text
...
// UpdateTag 更新标签
func (t *PostgresTxStore) UpdateTag(ctx context.Context, id string, updates *TagUpdate) error {
	query := `
		UPDATE tags SET
		...
		WHERE id = $1
	`
	result, err := t.tx.Exec(ctx, query, id, updates.Name, updates.Color, updates.Description)
	if err != nil {
...
// DeleteTag 删除标签
func (t *PostgresTxStore) DeleteTag(ctx context.Context, id string) error {
...
func (t *PostgresTxStore) AddTagToColumn(ctx context.Context, columnID string, tagID string) error {
	query := `
	...
	`
	id := uuid.New().String()
	_, err := t.tx.Exec(ctx, query, id, columnID, tagID)
	if err != nil {
...
func (t *PostgresTxStore) RemoveTagFromColumn(ctx context.Context, columnID string, tagID string) error {
	query := `DELETE FROM column_tags WHERE column_id = $1 AND tag_id = $2`
	result, err := t.tx.Exec(ctx, query, columnID, tagID)
	if err != nil {
...
func (t *PostgresTxStore) GetColumnTags(ctx context.Context, columnID string) ([]*TagRow, error) {
	query := `
		SELECT t.id, t.name, t.color, t.description, t.created_at::text, t.updated_at::text
		FROM tags t
...
func (t *PostgresTxStore) SearchColumnsByTag(ctx context.Context, tagID string, limit int) ([]*ColumnSearchRow, error) {
	if limit <= 0 {
		limit = 20
	}
	query := `
		...
		LIMIT $2
	`
	rows, err := t.tx.Query(ctx, query, tagID, limit)
	if err != nil {
